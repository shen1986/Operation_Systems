...
- 此指令的有趣特性是0x66不是操作码。0x01是操作码。那么，0x66是什么？回想一下，对于每个汇编指令，都有一个可选的指令前缀，这就是0x66。根据英特尔手册第1卷：
    + 操作数大小覆盖前缀允许程序在16位和32位操作数大小之间切换。两种大小都可以是默认值；使用pre-fix可以选择非默认大小。
...
- 
    + 索引是数组的偏移量。
    + 规模是指数的一个因素。Scale是值1、2、4或8之一；任何其他值都无效。若要使用2、4或8以外的值进行缩放，必须将比例因子设置为1，并且必须手动计算偏移。例如，如果要获取数组中第n个元素的地址，并且每个元素的长度为12字节。因为每个元素都是12字节长，而不是1、2、4或8，所以Scale设置为1，编译器需要计算偏移量：
    ```
