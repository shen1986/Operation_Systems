...
- 指令前缀出现在指令的开头。前缀是可选的。程序员可以选择是否使用前缀，因为实际上，所谓的前缀只是另一个汇编指令
...
- 插入到另一个可应用此前缀的汇编指令之前。默认情况下，带有2或3字节操作码的指令包含前缀。
...
- 0xff是操作码。旁边的0x26是ModR/M字节。在16位表中查找，第一个操作数在行中，相当于disp16，表示16位偏移量。由于指令没有第二个操作数，因此可以忽略该列。
...
```
- 那么机器代码是：
- 66&emsp;01&emsp;<font color=red>c8</font>

- 此指令的有趣特性是0x66不是操作码。0x01是操作码。那么，0x66是什么？回想一下，对于每个汇编指令，都有一个可选的指令前缀，这就是0x66。根据英特尔手册第1卷：
    + 操作数大小覆盖前缀允许程序在16位和32位操作数大小之间切换。两种大小都可以是默认值；使用前缀可以选择非默认大小。
- 如果CPU切换到32位模式，当它运行带有0x66前缀的指令时，指令操作数仅限于16位宽度。另一方面，如果CPU处于16位环境中，则32位被视为非标准，因此，指令操作数被临时升级到32位宽度，而没有预先修复的指令使用16位操作数。
- 旁边的<font color=red>c8</font>是ModR/M字节。在16位表中查找c8值，行告诉第一个操作数是ax，列告诉第二个操作数是cx；不能忽略列，因为第二个操作数在指令中。
- 为什么行中的第一个操作数是列中的第二个操作数？让我们将ModR/M字节（示例值为c8）分解为位：

<table>
    <tr>
        <th colspan="2">mod</th>
        <th colspan="3">reg/opcode</th>
        <th colspan="3">r/m</th>
    </tr>
    <tr>
        <td>1</td>
...
- mod字段将寻址模式分为4种不同的类别。进一步结合r/m字段，可以从24行中选择一种寻址模式。如果指令只需要一个操作数，则可以忽略该列。然后，如果指令需要，reg/opcode字段最终会提供一个额外的寄存器或不同的变量。

- SIB是缩放索引基字节。此字节编码将内存位置计算到数组元素中的方法。SIB是基于此公式计算有效地址的名称：
```
Effective address = scale ∗ index + base
```
- 
    + index是数组的偏移量。
    + scale是指数的一个因素。Scale是值1、2、4或8之一；任何其他值都无效。若要使用2、4或8以外的值进行缩放，必须将比例因子设置为1，并且必须手动计算偏移。例如，如果要获取数组中第n个元素的地址，并且每个元素的长度为12字节。因为每个元素都是12字节长，而不是1、2、4或8，所以Scale设置为1，编译器需要计算偏移量：
    ```
    Effective address = 1 ∗ (12 ∗ n) + base
    ```
    + 当我们可以手动计算偏移量时，为什么还要使用SIB呢？答案是，在上面的场景中，必须执行额外的mul指令来获取偏移量，mul指令消耗超过1个字节，而SIB只消耗1个字节。更重要的是，如果元素在一个循环中被多次重复访问，例如数百万次，那么额外的mul指令可能会损害性能，因为CPU必须花费时间执行数百万条额外的mul指令。值2、4和8不是随机选择的。它们映射到16位（或2字节）、32位（或4字节）和64位（或8字节）的数字，这些数字通常用于密集的数值计算。
    + base是起始地址。

- 下表列出了SIB byte的所有256个值，查找规则类似于ModR/M表：
...
```
- 生成以下代码：
- 00000000&emsp;67&emsp;ff&emsp;24&emsp;<font color=red>43</font>
- 首先，第一个字节0x67不是操作码，而是前缀。数字是地址大小覆盖前缀的预定义前缀。在预修复之后，是操作码0xff和ModR/M字节0x24。MODR/M的值表明存在一个如下的SIB字节。SIB字节是0x43。
- 在SIB表中查找，行告诉eax按2缩放，列告诉要添加的基在ebx中。
...
- 如果MOD是00B，[*]表示disp32没有基。否则，[*]表示disp8或disp32+[EBP]。这提供了以下地址模式：

图片

- 生成的机器代码是：
- ff&emsp;26&emsp;<font color=red>34</font>&emsp;<font color=red>12</font>
- 0x1234在原始机器代码中生成为3412，它是位移，位于0x26旁边，0x26是ModR/M字节。
...
```
- 生成机器代码：
- 67&emsp;ff&emsp;24&emsp;85&emsp;<font color=red>34</font>&emsp;<font color=red>12</font>&emsp;<font color=red>00</font>&emsp;<font color=red>00</font>
    + 0x67是地址大小重写前缀。其含义是，如果指令运行默认地址大小（如16位），则使用前缀可使指令使用非默认地址大小（如32位或64位）。由于二进制应该是16位，0x67将指令更改为32位模式。
    + 0xff是操作码。
    + 0x24是ModR/M字节。根据表4.5.2，该值表示后面跟着一个SIB字节。
    + 0x85是SIB字节。根据表4.5.3，字节0x85可以被分解成如下位：

    图片

    + 上述值分别通过SS列、R/M列和REG的8列得到。总位合并为值10000101，十六进制值为0x85。默认情况下，如果未指定位移后的寄存器，则将其设置为EBP寄存器，因此始终选择第6列（位模式101）。如果示例使用另一个寄存器：

    + 例4.5.7。例如：
    ```
    jmp [eax * 4 + eax + esi]
    ```
    + SIB字节变为0x86，而不是第7列中的。再次尝试用表4.5.3进行验证。

    + 34&emsp;12&emsp;00&emsp;00是位移。可以看到，由于地址大小覆盖前缀，位移的大小为4字节，相当于32位。

- 当指令接受固定值（例如0x1234）作为操作数时，此可选字段立即保存该值。请注意，此字段与置换不同：该值不必使用偏移量，而是任意值。
...
```
- 生成代码：
- 66&emsp;b8&emsp;34&emsp;12&emsp;00&emsp;00
    + 0x66是操作数大小的重写前缀。与地址大小覆盖前缀类似，此前缀使操作数大小成为非默认值。
    + 0xb8是mov指令的操作码之一。
    + 0x1234是要存储在寄存器eax中的值。它只是一个直接存储到寄存器中的值，仅此而已。另一方面，位移值是某些地址计算的偏移量。
- 练习4.5.1。阅读第二卷第2.1节了解更多细节。
- 练习4.5.2。浏览第一卷第5.1节。阅读第一卷第七章。如果有些术语你不懂，例如分词，不要担心，因为这些术语将在后面的章节中解释或忽略。

6. 详细理解说明
- 在指令参考手册（第2卷）中，从第3章开始，每个x86指令都有详细的文档记录。每当需要精确的指令时，我们总是先查阅本文档。然而，在使用文件之前，我们必须先了解书面协议。每条指令都有以下用于组织信息的公共结构：
